...
$ReleaseVersion = (Get-Content -Path ".\UDPBD-for-XEB+-CLI\UDPBD-for-XEB+-CLI.csproj" | Select-String -Pattern AssemblyVersion).ToString().Trim() -replace "<[^>]+>"
$ReleaseFolder = "$GUIDir\release-$ReleaseVersion"
$XEBPCLIFolder = "$ReleaseFolder\UDPBD-XEB-CLI"
...
New-Item -ItemType Directory -Path $XEBPCLIFolder
...
Copy-Item -Path "..\List Builder\vmc_groups.list" -Destination $XEBPCLIFolder -Force
Get-ChildItem -File -Path "$CLIDir\*" | Move-Item -Destination $XEBPCLIFolder -Force
Get-ChildItem -File -Path "$GUIDir\*" | Move-Item -Destination $UDPBDGFolder -Force

Copy-Item -Path ".\Needed-for-Release\*" -Exclude *.txt -Destination $XEBPCLIFolder
Copy-Item -Path ".\Needed-for-Release\*" -Include *.txt -Destination $ReleaseFolder
Copy-Item -Path "..\udpfs_server" -Exclude *.txt -Destination $UDPBDGFolder -Recurse
