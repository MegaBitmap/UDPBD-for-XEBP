...
internal class VMC
{
    public static async Task<bool> Sync(string gamePath, List<string> gameList, string listType, Label logLabel, Panel logPanel)
    {
        ...
        foreach (var game in gameList)
        {
            ...
            string vmcFullPath;
            byte[] currentVmc = Resources.VMC8_ZIP;
            ...
            if (crossSaveIDs.Contains(serialID))
            {
                ...
                string currentGroup = "";
                foreach (string line in VMCList)
                {
                    if (checkSize)
                    {
                        checkSize = false;
                        if (line == "32")
                        {
                            currentVmc = Resources.VMC32_ZIP;
                            currentVmcSize = 32;
                        }
                        else
                        {
                            currentVmc = Resources.VMC8_ZIP;
                            currentVmcSize = 8;
                        }
                    }
                ...
                {
                    SyncSNL.WriteLine($"Failed to find a group for {serialID}", logLabel, logPanel);
                    return false;
                }
            ...
            if (!File.Exists(vmcFullPath))
            {
                ...
                {
                    Stream vmcStream = new MemoryStream(currentVmc);
                    await ZipFile.ExtractToDirectoryAsync(vmcStream, "temp");
                    File.Move($"temp/BlankVMC{currentVmcSize}.bin", vmcFullPath);
                    SyncSNL.WriteLine($"Created {vmcRelativePath} for {game}", logLabel, logPanel);
                }
                else
                {
                    SyncSNL.WriteLine("Not enough space to create a new VMC file.", logLabel, logPanel);
                    return false;
                }
            }
        }
        ...
        if (listType == "XEBP")
        {
            string hash = SyncXEBP.ComputeMD5(string.Join("\n", gameListVMC));
            gameListVMC.Add(hash);
            File.WriteAllLines("temp/neutrinoUDPBD.list", gameListVMC);
        }
        else
            File.WriteAllLines("temp/UDPBDList.txt", gameListVMC);

        return true;
    }

    private static readonly string[] VMCList =
    [
        "XEBP_000.02",
        "SLPM_650.86",
        "SLPM_650.87",
